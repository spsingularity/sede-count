...
\title{A capacity theorem for long-range entanglement networks: the
area--volume dichotomy and a bound on Barrow entropy}
...
\begin{abstract}
We model the degrees of freedom of a gravitational horizon as a network
of Planck-density sites, each with a bounded total entanglement weight
\(\kappa\) --- a finite local Hilbert-space dimension --- and bond
weights decaying with separation as \(w(r) \propto  r^{-\alpha }\).
Three results follow, each a theorem about this model alone. First, a
capacity theorem: the bond-cut capacity of any region \(\Omega\) obeys
\(C(\Omega ) \leq  \kappa\)\textbar{}\(\Omega\)\textbar, so the Barrow
entropy deformation satisfies \textbf{\(\Delta  \leq  1\)} identically;
no fractal-horizon geometry is needed to bound it, and for long-range
couplings \((\alpha  < d)\) none is available, since the minimum cut is
not a surface. Second, an area--volume dichotomy: the capacity is the
enclosed \emph{volume} \((\Delta  = 1)\) for every range \(\alpha  < d\)
and the boundary \emph{area} \((\Delta  = 0)\) for \(\alpha  > d + 1\),
with the crossover at \(\alpha  = d\) exactly, and the volume basin is
derived with an explicit finite-size error bound, uniform in position;
...
selecting a parameter --- DESI DR3 and Euclid decide \(\Delta  = 1\)
versus \(\Delta  = 0\) at a forecast \(\sigma (\Delta ) \approx  0.09\).
Third, \(\Delta  = 1\) exactly is the infrared-cutoff endpoint L → ∞. We
state the limits explicitly: attaining \(\Delta  = 1\) requires, beyond
the \(\alpha  < d\) geometry, a cut-saturating horizon \emph{state},
which we identify as the tracial state of the site algebra, and whether
a physical horizon is driven to it is left open; the L → ∞ endpoint is a
stated choice, not a derivation; what the network counts is a structural
capacity, not manifestly the generalized entropy; and the volume
identification is the non-default, empirically-bet-on branch. This
shortens --- it does not eliminate --- the assumption list behind the
volume-law postulate of Structural Entropy Dark Energy. The results of
§2 are self-contained: the capacity theorem, the dichotomy and the
crossover are statements about the network model of §2.1 alone, are
state-independent, and take no input from the companion papers cited
here, which supply only the cosmological motivation for the question and
the state half of its interpretation.
\end{abstract}

\maketitle

\noindent\textbf{Keywords:} holographic dark energy; horizon
...
How much entropy can a region of a gravitational horizon carry, and does
that count scale with the enclosed \emph{volume} or with the bounding
\emph{area}? We answer this inside a concrete model. The horizon degrees
of freedom are a network with a finite per-site \emph{leg budget}
\(\kappa\) --- equivalently, a finite local Hilbert-space dimension ---
and bond weights that decay as a power, \(w(r) \propto  r^{-\alpha }\).
From this single premise three results follow, each a statement about
the network alone: a capacity theorem bounding the Barrow deformation by
\(\Delta  \leq  1\), an area--volume dichotomy with an exact crossover
at \(\alpha  = d\) that places gravity's 1/r reach deep in the volume
phase, and the identification of \(\Delta  = 1\) as the infrared-cutoff
endpoint L → ∞. The network cut-scaling that underlies the dichotomy is
a question in long-range percolation and small-world graph theory; what
is new here is reading a \emph{horizon} entropy count off it.

The question is not idle, and the motivation for asking it is
cosmological. Structural Entropy Dark Energy (SEDE) reproduces the
cosmological data with no fitted dark-sector parameter, at the cost of a
single foundational input: the cosmic apparent horizon carries
...
calibration. This paper reduces that count to a shorter list of stated
assumptions; it does not fully discharge it.

The three results in detail (Figure 1), each machine-checked
(§Reproducibility):
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{../../results/count_fig0_reduction}}
...
\begin{equation}w_{ij} \propto r_{ij}^{-\alpha}, \qquad \sum_j w_{ij} = \kappa \quad\text{for every } i. \tag{2.1}\end{equation}
...
\begin{equation}C(\Omega) = \sum_{i\in\Omega,\, j\notin\Omega} w_{ij}, \tag{2.2}\end{equation}
...
\begin{equation}C(\Omega) \le \sum_{i\in\Omega} \sum_j w_{ij} = \kappa\,|\Omega|. \tag{2.3}\end{equation}
...
\begin{equation}\Delta \le 1. \tag{2.4}\end{equation}
...
\begin{equation}\int^L u^{d-1-\alpha}\,du \quad\text{versus}\quad \int^\rho u^{d-1-\alpha}\,du. \tag{2.5}\end{equation}
...
\begin{equation}C(\rho) \to \kappa\, n\, V(\rho) \qquad \text{(volume law, budget saturated).} \tag{2.6}\end{equation}
...
\begin{equation}f_{\rm out}(x) \;\ge\; 1 - K(d,\alpha)\,(\rho/L)^{\,d-\alpha}, \qquad
K(d,\alpha) = \frac{S_{d-1}\,2^{\,d-\alpha}/(d-\alpha)}{c_d\,(1-2^{-d})\,2^{-\alpha}},\end{equation}
with \(S_{d-1}\) the unit-sphere surface and \(c_{\mathrm{d}}\) the
unit-ball volume. Consequently
\begin{equation}\kappa\,n\,V(\rho)\left[1 - K(d,\alpha)(\rho/L)^{d-\alpha}\right] \;\le\; C(\rho) \;\le\; \kappa\,n\,V(\rho). \tag{2.6a}\end{equation}
...
(\texttt{src/\allowbreak outward\_\allowbreak fraction\_\allowbreak bound.\allowbreak py}, d = 3, L = 200, exact angular
...
\begin{equation}s_0 = \kappa\, n = \kappa/\ell_P^3, \tag{2.7}\end{equation}
...
\begin{equation}w(r)\ \text{decays slower than}\ r^{-d}. \tag{2.8}\end{equation}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{../../results/count_fig1_dichotomy}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{../../results/count_fig2_membrane}}
...
\(\bar{R}^2\)\(∫d\Omega [(1+h)^{2}\)+½\textbar{}\(\nabla _{\mathrm{S}}\)
h\textbar² + O(\textbar{}\(\nabla _{\mathrm{S}}\) h\textbar⁴){]} carries
no cubic term, the constraint reads

\begin{equation}\nabla_S^2 h + 2h + 2h^2 = 2\mathcal{S}, \qquad \mathcal{S} = \Phi + \Psi + \dot\Psi/H - H^{-1}\partial_R \Psi. \tag{2.9}\end{equation}
...
apparent horizon. On the round 2-sphere (areal radius \(\bar{R}\) = 1/H)
the AMS operator L \(\psi  = -\nabla ^{2}_{\mathrm{S}}\) \(\psi  + 2\)
\(s^{A}\) \(D_{\mathrm{A}}\)
...
\textbar A\textbar² = 2/\(\bar{R}^2\), i.e.~exactly
\(-(1\)/\(\bar{R}\))\((\nabla ^{2}_{\mathrm{S}} + 2)\). Its spectrum is
the published one, ℓ\((\ell +1) - 2\), and the \textbf{ℓ = 1 kernel is
the translation zero mode every MOTS stability operator carries} --- not
a coincidence but the sphere's freedom to be displaced. We reproduce
this independently, by a second geometric route (surface-of-revolution
...
symbolic algebra (\texttt{src/\allowbreak ams\_\allowbreak stability.\allowbreak py}).
...
\begin{equation}1 - s(\rho)/\kappa = D(x) = c\, x^{d-\alpha}, \qquad x \equiv \rho/L. \tag{5.1}\end{equation}
...
\begin{equation}c = \lim_{x\to0} \frac{D(x)}{x^2} = 0.6730 \pm 0.0001,\end{equation}
...
\begin{equation}\Delta(x) = \frac{d\ln S}{d\ln\rho} - 2 = 1 - \frac{x\, D'(x)}{1 - D(x)} = 1 - \frac{(d-\alpha)\, c\, x^{d-\alpha}}{1 - c\, x^{d-\alpha}}. \tag{5.2}\end{equation}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{../../results/count_fig3_cutoff}}
...
\begin{equation}\Delta = 1 \quad\text{exactly.} \tag{5.3}\end{equation}
...
state assigns
\begin{equation}S(\Omega) \;=\; \log \dim \mathcal{H}_\Omega \;=\; \kappa|\Omega| \qquad \text{for \emph{every} } \Omega,\end{equation}
...
\begin{equation}s_0 = \kappa n = \kappa/\ell_P^3 \tag{6.1}\end{equation}
...
\begin{equation}S = f_{\rm sat}\, s_0\, V, \tag{6.2}\end{equation}
...
\begin{equation}C_{\rm str}\ \longrightarrow\ q_R = \sigma_R^2\ \longrightarrow\ u(q_R),\end{equation}
...
\tightlist
\item
  \texttt{src/\allowbreak mincut\_\allowbreak membrane.\allowbreak py} --- maximum-flow minimum cut on the
  lattice ball; membrane-thickness and shell-fraction diagnostics
  (§\ref{sec-2-5}, tables i--ii).
\item
  \texttt{src/\allowbreak capacity\_\allowbreak radial.\allowbreak py} --- spherical reduction with the
  ...
  \(\alpha  > d + 1\)), asserted.
\item
  \texttt{src/\allowbreak outward\_\allowbreak fraction\_\allowbreak bound.\allowbreak py} --- the finite-size
  ...
  asserted.
\item
  \texttt{src/\allowbreak edge\_\allowbreak deficit.\allowbreak py} --- Newton-kernel solve at
  \(\alpha  = 1\); the deficit law and the convergence of c
  (§\ref{sec-5-1}).
\item
  \texttt{src/\allowbreak horizon\_\allowbreak constraint.\allowbreak py} --- symbolic expansion of the
  apparent-horizon condition, eq. (2.9); verified against the exact mean
  curvature of a translated sphere.
\item
  \texttt{src/\allowbreak ams\_\allowbreak stability.\allowbreak py} --- external-standard check: eq. (2.9)
  ...
  −(1/\(\bar{R}\))\((\nabla ^{2}_{\mathrm{S}} + 2)\), the published
  spectrum ℓ\((\ell +1) - 2\) with the ℓ = 1 translation zero mode, and
  the AMS matter term \(8\pi\) \(T_{ab}\ell ^{a}\) \(k^{b} = 3H^{2}\)+Ḣ
  that isolates the slicing caveat.
\item
  \texttt{run\_\allowbreak all.\allowbreak py} --- single entry point; runs every stage with
  validation assertions.
\item
  \texttt{figures/\allowbreak make\_\allowbreak figures.\allowbreak py} --- regenerates the four figures
  from the same \texttt{src/\allowbreak } computations (Figs 1--4 →
...
\href{https://doi.org/10.5281/zenodo.21525522}{10.5281/zenodo.21525522}.
Every quantitative claim is reproduced from a single entry point
(\texttt{run\_\allowbreak all.\allowbreak py}), each stage carrying validation assertions that
all pass, and the four figures regenerate from the same computations via
\texttt{figures/\allowbreak make\_\allowbreak figures.\allowbreak py}. No observational data were generated
