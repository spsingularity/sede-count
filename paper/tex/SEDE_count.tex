...
\title[The horizon count from a leg-budgeted network]{A capacity theorem
for horizon entropy: Barrow \(\Delta  \leq  1\) from a long-range
...
needs the \(\alpha  < d\) volume-basin geometry, derived here, and a
cut-saturating horizon \emph{state} --- the one assumption the theorem
does not remove) --- no fractal-horizon geometry is needed, and for
...
postulate of Structural Entropy Dark Energy.\end{abstract}
...
leg-budgeted network of §\ref{sec-2-1} (and, for the \(\alpha  = d\)
crossover of §\ref{sec-2-3}, one limit interchange remains unproven ---
§\ref{sec-8}), not about an arbitrary horizon. Two consequences are
worth stating plainly. First, \(\Delta  > 1\) is impossible in this
framework, so a high-significance measurement of \(\Delta  > 1\)
falsifies it with no parameter available to absorb the result. Second,
whatever selects \(\Delta  = 1\) must be a \emph{saturation} argument,
since (2.3) is the only bound in play.
...
symbolic algebra (\texttt{ams\_stability.py}).
...
\textbf{What is not settled.} Two things, stated plainly. First, the
\(\alpha  = d\) crossover of §\ref{sec-2-3} is physics-rigorous but not
yet a theorem: the dominance argument on the outward-fraction integral
(2.5) is correct term by term, but the interchange of the \(\rho\) → ∞
and continuum limits is not justified, and a mathematically-minded
reader will want it bounded --- this is the one place the
\emph{derivation}, as opposed to the numerics, is incomplete. Second,
and more fundamental, the identification S = C is the state question
(§\ref{sec-6-1}): the network gives the count, the maximally-scrambled
state gives that the count is attained. That assumption is route A of
the foundations paper, and it is the honest residue of the whole
programme --- now a single question, and one shared with area-law black
holes (a black hole carries the same maximally-entangled horizon state;
what differs is that it is undriven, so its capacity is the boundary
count, §\ref{sec-6-2}). The open content of SEDE is thereby relocated,
in its entirety, from the \emph{count} to the \emph{state} --- and the
state half is settled, independently of SEDE, by the scrambling /
Page-curve physics the foundations paper cites.
...
\item
  \texttt{make\_figures.py} --- regenerates the four figures from the
  same \texttt{src/} computations (Figs 1--4 →
...
\texttt{{[}Zenodo\ DOI\ —\ to\ be\ assigned{]}}. Every quantitative
claim is reproduced from a single entry point (\texttt{run\_all.py},
also wired into the repository-wide \texttt{reproduce\_all.py} as the
\texttt{netcount} stage), each stage carrying validation assertions that
all pass, and the four figures regenerate from the same computations via
\texttt{make\_figures.py}. No observational data were generated or
analysed beyond the standard public inputs cited in the companion
papers.
